...
export const Navbar = () => {
    ...
    const onSuggestionSelected = (event, { suggestion }) => {
		...
		if (store.people.some(person => person.uid === suggestion.uid)) {
			type = "character";
		} else if (store.planets.some(planet => planet.uid === suggestion.uid)) {
			type = "planet";
		} else if (store.vehicles.some(vehicle => vehicle.uid === suggestion.uid)) {
			type = "vehicle";
		}
	
		if (type) {
			navigate(`/details/${type}/${suggestion.uid}`);
		}
	};
